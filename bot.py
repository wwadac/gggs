...
async def start_handler(event):
    keyboard = [
        ...
        [Button.inline("Инфо об аккаунте", b'account_info')]
    ]
...
async def callback_handler(event):
    ...
    if event.data == b'my_id':
        ...
        await event.edit("📩 Перешлите сообщение от нужного пользователя или напишите его @username")
    
    elif event.data == b'account_info':
...
async def message_handler(event):
    if event.is_reply:
        ...
        try:
            user = await client.get_entity(event.message.text)
